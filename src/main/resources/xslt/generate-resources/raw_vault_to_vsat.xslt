...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/raw_vault/db_sources">
        ...
        <xsl:for-each select="db_source/table/stage/(hub|link)/sat">
            <xsl:call-template name="genViewsOnSats"/>
        </xsl:for-each>
        <xsl:call-template name="genVSatPropertiesYml"/>
    </xsl:template>

    <xsl:template name="genViewsOnSats">
        ...
        <xsl:variable name="vsat_name">
            <xsl:choose>
                <xsl:when test="local-name(..) = 'hub'">
                    <xsl:value-of select="concat('vsat_', $stage_name, '_', $hub_name)"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="concat('vsat_', $stage_name, '_', $link_name)"/>
                </xsl:otherwise>
            </xsl:choose>
        ...
        <xsl:result-document href="{concat('file://', $baseDir, '/target/classes/DataVault/models/s04_raw_vault/vsat/', $vsat_name,'_seen.sql')}" method="text" omit-xml-declaration="yes">
            <xsl:text>&#xA;</xsl:text>
            ...
            <xsl:choose>
                <xsl:when test="local-name(..) = 'hub'">
                    <xsl:text>ON hub.</xsl:text><xsl:value-of select="$hub_key_prefix"/><xsl:text>_PK</xsl:text>
                    <xsl:text> = sat.</xsl:text><xsl:value-of select="$hub_key_prefix"/><xsl:text>_PK</xsl:text>
                </xsl:when>
                <xsl:otherwise>
        ...
        <xsl:result-document href="{concat('file://', $baseDir, '/target/classes/DataVault/models/s04_raw_vault/vsat/',$vsat_name,'_last.sql')}" method="text" omit-xml-declaration="yes">
            <xsl:text>&#xA;</xsl:text>
    ...
    <xsl:template name="genVSatPropertiesYml">
        <xsl:result-document href="{concat('file://', $baseDir, '/target/classes/DataVault/models/s04_raw_vault/vsat/properties.yml')}" method="text" omit-xml-declaration="yes">
            <xsl:text>&#xA;</xsl:text>
            <xsl:text>version: 2&#xA;</xsl:text>
            <xsl:text>&#xA;</xsl:text>
            <xsl:text>models:&#xA;</xsl:text>
            <xsl:for-each select="db_source/table/stage/(hub|link)/sat">
                ...
                <xsl:text>    description: View on latest seen data from </xsl:text><xsl:value-of select="$sat_name"/><xsl:text>&#xA;</xsl:text>
