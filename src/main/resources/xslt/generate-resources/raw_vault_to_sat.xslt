...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/raw_vault/db_sources">
        ...
        <xsl:for-each select="db_source/table/stage/(hub|link)/sat">
            <xsl:call-template name="genSats"/>
        </xsl:for-each>
        <xsl:call-template name="genSatPropertiesYml"/>
    </xsl:template>

    <xsl:template name="genSats">
        ...
        <xsl:result-document href="{concat('file://', $baseDir, '/target/classes/DataVault/models/s04_raw_vault/sat/', $sat_name,'.sql')}" method="text" omit-xml-declaration="yes">
            <xsl:text>&#xA;</xsl:text>
            <!-- sats
            ...
            <xsl:text>, 'LOAD_DATE', '</xsl:text><xsl:value-of select="upper-case($sat_name)"/><xsl:text>_HASHDIFF']}&#xA;</xsl:text>
            <xsl:text>        ]&#xA;</xsl:text>
            <xsl:text>    )&#xA;</xsl:text>
            <xsl:text>}}&#xA;</xsl:text>
    ...
    <xsl:choose>
        ...
        <xsl:otherwise>
            ...
            <xsl:text>&#xA;</xsl:text>
            <xsl:text>{{ automate_dv.sat(src_pk=src_pk, src_hashdiff=src_hashdiff,&#xA;</xsl:text>
            <xsl:text>                src_payload=src_payload, src_eff=src_eff,&#xA;</xsl:text>
            <xsl:text>                src_ldts=src_ldts, src_source=src_source,&#xA;</xsl:text>
            <xsl:text>                source_model=source_model) }}&#xA;</xsl:text>
        </xsl:result-document>
    </xsl:template>

    <xsl:template name="genSatPropertiesYml">
        <xsl:result-document href="{concat('file://', $baseDir, '/target/classes/DataVault/models/s04_raw_vault/sat/properties.yml')}" method="text" omit-xml-declaration="yes">
            <xsl:text>&#xA;</xsl:text>
            <xsl:text>version: 2&#xA;</xsl:text>
            <xsl:text>&#xA;</xsl:text>
            <xsl:text>models:&#xA;</xsl:text>
            <xsl:for-each select="/raw_vault/db_sources/db_source/table/stage/(hub|link)/sat">
                <xsl:variable name="stage_name" select="../../@name"/>
                <xsl:variable name="hub_name" select="concat('hub_', ..[local-name() = 'hub']/nat_key/@idref)"/>
                <xsl:variable name="link_name" select="concat('link_', string-join(../nat_key/@idref, '_'))"/>
